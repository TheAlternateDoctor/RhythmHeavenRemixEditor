...
import io.github.chrislo27.rhrefresh.editor.ClickOccupation
import io.github.chrislo27.rhrefresh.editor.Editor
import io.github.chrislo27.rhrefresh.editor.Tool
import io.github.chrislo27.rhrefresh.editor.picker.*
import io.github.chrislo27.rhrefresh.editor.quickswitch.SwitchToGame
import io.github.chrislo27.rhrefresh.editor.stage.playalong.PlayalongStage
import io.github.chrislo27.rhrefresh.editor.stage.playalong.PlayalongToggleButton
import io.github.chrislo27.rhrefresh.editor.stage.theme.ThemeChooserStage
import io.github.chrislo27.rhrefresh.entity.model.IEditableText
...
import io.github.chrislo27.rhrefresh.patternstorage.FileStoredPattern
import io.github.chrislo27.rhrefresh.patternstorage.StoredPattern
...
import io.github.chrislo27.rhrefresh.util.OSUtils
...
class EditorStage(parent: UIElement<EditorScreen>?,
                  camera: OrthographicCamera, val main: RHREfreshApplication, val editor: Editor)
    ...
    init {
        ...
        messageBarStage = Stage(this, camera).apply {
            this.location.set(0f, 0f,
                              1f, Editor.MESSAGE_BAR_HEIGHT / RHREfresh.HEIGHT.toFloat())
        ...
        buttonBarStage = Stage(this, camera).apply {
            this.location.set(screenX = (Editor.BUTTON_PADDING / RHREfresh.WIDTH),
                              screenY = 1f - ((Editor.BUTTON_PADDING + Editor.BUTTON_SIZE) / RHREfresh.HEIGHT),
                              screenWidth = 1f - (Editor.BUTTON_PADDING / RHREfresh.WIDTH) * 2f,
                              screenHeight = Editor.BUTTON_SIZE / RHREfresh.HEIGHT)
        }
        pickerStage = object : Stage<EditorScreen>(this@EditorStage, camera) {
            ...
            this.location.set(screenY = messageBarStage.location.screenY + messageBarStage.location.screenHeight,
                              screenHeight = ((Editor.ICON_SIZE + Editor.ICON_PADDING) * Editor.ICON_COUNT_Y + Editor.ICON_PADDING) / RHREfresh.HEIGHT
                             )
        ...
        minimapBarStage = Stage(this, camera).apply {
            this.location.set(screenY = pickerStage.location.screenY + pickerStage.location.screenHeight,
                              screenHeight = Editor.ICON_SIZE / RHREfresh.HEIGHT)
            this.elements += ColourPane(this, this).apply {
                this.colour.set(Editor.TRANSLUCENT_BLACK)
            }
            this.elements += ColourPane(this, this).apply {
        ...
        centreAreaStage = Stage(this, camera).apply {
            ...
            this.location.set(
                    screenHeight = (buttonBarStage.location.screenY - this.location.screenY - (Editor.BUTTON_PADDING / RHREfresh.HEIGHT)))
            this.updatePositions()
        ...
        run buttonBar@{
            ...
            buttonBarStage.elements +=
                    ColourPane(buttonBarStage, buttonBarStage).apply {
                        this.colour.set(Color(0.4f, 0.4f, 0.4f, 0.8f))
                        this.location.set(
